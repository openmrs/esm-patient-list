...
const setup = async () => {
  ...
  const systemUuid = await createPatientList('waiting list', PATIENT_LIST_TYPE.SYSTEM);
  ...
  const systemUuid2 = await createPatientList('bathroom black list', PATIENT_LIST_TYPE.USER);
